...
const AddProductCategoryPage = () => {
  ...
  const handleOpenApiDialog = (index) => {
    ...
    console.log('Opening API Dialog for index:', index, 'API Details:', currentApiDetails);
  ...
  const removeHeader = (index) => {
    const filteredHeaders = apiDetails.headers.filter((_, idx) => idx !== index);
  ...
  const fieldTemplates = {
    ...
    Hotel: [
      ...
      { key: 'description', type: 'String', required: false, validations: {} },
      {
        ...
        required: false,
        validations: {},
        apiDetails: { apiUrl: '', headers: [], method: 'GET', payload: '', responseParser: [] },
      },
    ],
    Room: [
      { key: 'name', type: 'String', required: true, validations: { unique: true } },
  ...
  const addField = () => {
    ...
    console.log('Adding new field with responseParser:', newField.apiDetails.responseParser);
  ...
  const handleSubmitApi = (e) => {
    ...
    const enhancedCustomFields = customFields.map((field) => {
      return {
        ...field,
        external: field.external && field.apiDetails && field.apiDetails.apiUrl ? true : undefined,
      };
    });
    ...
    console.log('Initial Transformed Payload:', JSON.stringify(transformedPayload, null, 2));
  ...
  const removeParser = (index) => {
    const filteredParsers = apiDetails.responseParser.filter((_, idx) => idx !== index);
  ...
  return (
    ...
    >
      {Object.keys(data).length == 0 && (
        <Paper elevation={3} style={{ padding: '20px' }}>
          <div className="flex justify-between">
            ...
            <Button onClick={handlePressManageButton} style={{ height: '30px' }}>
              Manage Fields
            </Button>
          </div>
          <form onSubmit={handleSubmitApi}>
            <TextField
              ...
              required
            />
            {categoryErrorName && <p style={{ color: 'red' }}>{categoryErrorName}</p>}
          ...
          <Dialog open={apiDialogOpen} onClose={handleCloseApiDialog}>
            ...
            <DialogContent>
              <TextField
                ...
                onChange={(e) => handleApiDetailsChange('apiUrl', null, null, e.target.value)}
              />
              {apiDetails.headers.map((header, index) => (
                ...
                >
                  <TextField
                    ...
                    value={header.key}
                    onChange={(e) =>
                      handleApiDetailsChange('headers', index, 'key', e.target.value)
                    }
                  ...
                  <TextField
                    ...
                    value={header.value}
                    onChange={(e) =>
                      handleApiDetailsChange('headers', index, 'value', e.target.value)
                    }
                    style={{ flex: 1 }}
                  />
              ...
              {apiDetails?.responseParser.map((parser, index) => (
                ...
                >
                  <TextField
                    ...
                    value={parser.key}
                    onChange={(e) =>
                      handleApiDetailsChange('responseParser', index, 'key', e.target.value)
                    }
                    style={{ marginRight: '10px', flex: 1 }}
                  />
                  <TextField
                    ...
                    value={parser.value}
                    onChange={(e) =>
                      handleApiDetailsChange('responseParser', index, 'value', e.target.value)
                    }
                    style={{ flex: 1 }}
                  />
              ...
              <TextField
                ...
                onChange={(e) => handleApiDetailsChange('payloadBody', null, null, e.target.value)}
              />
            </DialogContent>
            <DialogActions>
      ...
      {Object.keys(data).length > 0 && (
        <Paper elevation={3} style={{ padding: '20px' }}>
          <div className="flex justify-between">
            <Box display="flex" alignItems="center" gap={2} sx={{ marginBottom: 2, marginTop: 2 }}>
              <Typography sx={{ fontWeight: 'medium' }} variant="h6" component="div">
                Entity Name:
              </Typography>
              <Typography style={{ marginTop: '3px' }} variant="subtitle1" component="span">
