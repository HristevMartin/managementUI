...
const Package = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategoryData = async () => {
      try {
        const response = await categorydata(); // Assuming categorydata is the API call
  ...
  useEffect(() => {
    // Fetch search configuration data
    const fetchSearchConfigData = async () => {
      try {
        const response = await searchconfig(); // Assuming searchconfig is the API call
  ...
  useEffect(() => {
    ...
    const fetchPluralData = async () => {
      setLoading(true);
      try {
        const response = await pluraldata(selectedCategory);
  ...
  useEffect(() => {
    if (selectedCategory === "Hotel") {
      const fetchRoomData = async () => {
        setLoading(true);
        try {
          const response = await roomdata();
          setroomData(response); // Save room data to state
  ...
  const handleCategoryChange = (event) => {
    // This handles single selection, update the selectedCategory
    const selectedCategory = event.target.value;
    setSelectedCategory(selectedCategory);

    // This handles multiple selection logic
  ...
  const handleSelectChange = (e) => {
    ...
    setSelectedProduct(selectedOption); // Update state with selected option
  ...
  const handleInboundOutboundChange = async (event) => {
    ...
    if (selectedValue === "outbound") {
      ...
      try {
        ...
        const fetchOutboundFlights = async (searchCriteria) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h1 className="text-3xl font-bold mb-6">Package</h1>
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            className="h-5 w-5"
          />
          {/* Active is disabled and will always be true */}
        </div>
